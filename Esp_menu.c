...
// Define number of encoders based on config
    ...
    {
        // Create button
        button_config_t btn_cfg = {0}; // Zero init the config
        btn_cfg.type = BUTTON_TYPE_GPIO;
        gpio_button_config_t gpio_btn_cfg = {
        ...
        BSP_ERROR_CHECK_RETURN_ERR(iot_button_create(&btn_cfg, &gpio_btn_cfg, &encoder_btn_handles[i]));
